...
function encryptBuffer(chunk: Buffer, sKey: string) {
  const iv = randomBytes(16);
  const key = createHash('sha256').update(sKey).digest('base64').slice(0, 32);
...
export function decrypt(chunk: Buffer, sKey: string) {
  const iv = chunk.slice(0, 16);
  chunk = chunk.slice(16);
  let key = createHash('sha256').update(sKey).digest('base64').slice(0, 32);
